...
import { planStatus, needsRenewal, TONE } from './user-status';
...
export const SEGMENTS = [
  ...
  { value: 'sinPlan', label: 'Sin plan' },
];

export function matchesSegment(user, segment) {
  if (segment === 'todos') return true;
  if (segment === 'activos') return user.state === 'Activo';
  if (segment === 'sinPlan') return !user.plan;
